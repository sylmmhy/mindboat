...
export const EnhancedDistractionAlert: React.FC<EnhancedDistractionAlertProps> = ({
  ...
  // Log when alert becomes visible
  useEffect(() => {
    if (isVisible && !alertShownRef.current) {
      console.log('🚨 [ALERT] Alert is now visible:', {
        distractionType,
            ...
            <Card className="max-w-md w-full p-8 text-center">
              ...
              {import.meta.env.DEV && (
                <div className="mt-4 p-2 bg-gray-100 rounded text-xs text-left">
                  ...
                  <p>Distraction Type: {distractionType}</p>
                  <p>Alert Visible: {isVisible ? 'Yes' : 'No'}</p>
                  <p>Selected: {selectedResponse || 'None'}</p>
                </div>
              )}
            </Card>
          </motion.div>
