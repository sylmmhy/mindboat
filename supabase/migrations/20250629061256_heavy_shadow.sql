...
CREATE FUNCTION get_user_voyage_stats(user_id UUID)
RETURNS TABLE (
...
CREATE FUNCTION get_destinations_with_stats(user_id UUID)
RETURNS TABLE (
  id UUID,
  user_id UUID,
  original_task TEXT,
...
CREATE FUNCTION increment_distraction_count(voyage_id UUID)
RETURNS VOID AS $$
BEGIN
  UPDATE voyages 
...
CREATE FUNCTION get_distraction_patterns(user_id UUID, days_back INTEGER DEFAULT 30)
RETURNS TABLE (
...
CREATE FUNCTION get_voyage_distraction_summary(voyage_id UUID)
RETURNS TABLE (
...
CREATE FUNCTION cleanup_old_distraction_events(days_to_keep INTEGER DEFAULT 90)
RETURNS BIGINT AS $$
DECLARE
