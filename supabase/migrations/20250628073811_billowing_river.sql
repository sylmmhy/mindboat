/*
  ...
  1. Functions for user statistics and analytics
  2. Functions for distraction tracking and patterns
  3. Functions for voyage analytics
  4. Optimized queries for performance

  These functions provide efficient data aggregation and analytics
  without exposing complex queries to the client.
*/

-- Function to get user voyage statistics
CREATE OR REPLACE FUNCTION get_user_voyage_stats(user_id UUID)
RETURNS TABLE (
...
BEGIN
  ...
  FROM voyages
  WHERE voyages.user_id = get_user_voyage_stats.user_id
...
CREATE OR REPLACE FUNCTION get_destinations_with_stats(user_id UUID)
RETURNS TABLE (
  id UUID,
  user_id UUID,
  original_task TEXT,
...
BEGIN
  ...
  WHERE d.user_id = get_destinations_with_stats.user_id
...
CREATE OR REPLACE FUNCTION increment_distraction_count(voyage_id UUID)
RETURNS VOID AS $$
BEGIN
  UPDATE voyages 
  SET distraction_count = distraction_count + 1 
  WHERE id = increment_distraction_count.voyage_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Function to get distraction patterns
CREATE OR REPLACE FUNCTION get_distraction_patterns(user_id UUID, days_back INTEGER DEFAULT 30)
RETURNS TABLE (
...
BEGIN
  ...
  WITH distraction_data AS (
    ...
    WHERE v.user_id = get_distraction_patterns.user_id
...
CREATE OR REPLACE FUNCTION get_voyage_distraction_summary(voyage_id UUID)
RETURNS TABLE (
...
BEGIN
  ...
  WITH distraction_stats AS (
    ...
    FROM (
      ...
      WHERE distraction_events.voyage_id = get_voyage_distraction_summary.voyage_id
    ) subq
  )
  SELECT 
...
BEGIN
  ...
  FROM voyages v
  WHERE v.user_id = get_voyage_stats_by_date.user_id
    AND v.status = 'completed'
    AND v.start_time >= start_date
    AND v.start_time <= end_date;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Function to clean up old distraction events (for maintenance)
CREATE OR REPLACE FUNCTION cleanup_old_distraction_events(days_to_keep INTEGER DEFAULT 90)
RETURNS BIGINT AS $$
DECLARE
...
CREATE POLICY "Users can view own voyage analytics"
  ON voyage_analytics
  FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

ALTER VIEW voyage_analytics ENABLE ROW LEVEL SECURITY;